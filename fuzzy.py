...
import common as cmn  # Assuming cmn has trapezoid, triangular functions
...
class Fuzzifier:
    ...
    def calc_membership_values(self, crisp_input: dict()):
        '''
    ...
    def plot_membership_functions(self, x_range: tuple = (0, 1.5), num_points: int = 200):
        '''
        ...
        if self.crisp_input is not None:
            plt.axvline(x=self.crisp_input, color='red', linestyle='--', linewidth=2, label=f'Crisp Input = {self.crisp_input}')
            ...
            for component in rule:
                ...
                if var_name in self.input_fuzzifiers.keys():
                    ...
                    if fuzzifier.membership_values is None:
                        raise ValueError(f"Membership values not calculated for '{var_name}'")
...
class Defuzzifier:
    ...
    (Centre of sets of a symmetric output memberhsip functions)
    ...
    def defuzzify(self):
        ...
        for ling_var, peak_values in self.output_peaks.items():
            ...
            crisp_outputs[ling_var] = float(numerator / denominator if denominator > 0.0 else 0.0)
